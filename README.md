...
让 Codex 和 Claude Desktop 通知真正提醒你，尤其是在你听音乐的时候。

这是一个给 Codex 和 Claude Desktop 用的 Windows 11 托盘小工具。

它解决一个很具体的问题：Codex 或 Claude Desktop 有通知时，Windows 通知经常不够明显，尤其是你在听音乐、看视频或者把注意力放到别的窗口时，很容易错过。更麻烦的是，Windows 自带通知声音有时不会正常响，或者被音乐盖过去。

这个工具会监听 Windows 通知中心里的 Codex 和 Claude Desktop 通知。识别到目标应用有新通知后，它会用自己的进程播放一段更明显的提示音，并且可以在提示音播放时短暂降低其他应用的音量，让通知更容易被听见。

它的目标很窄：只让 Codex 和 Claude Desktop 通知更容易被听见。它不会提高目标应用的整个应用音量，也不会修改系统主音量。

## 功能

- 监听 Codex 和 Claude Desktop 的 Windows 通知。
- 识别到目标应用通知后播放额外提示音。
- 可选音频闪避：提示音播放时短暂降低其他应用音量。
- 托盘常驻，不显示主窗口。
- 中文托盘菜单。
- 可暂停或恢复通知增强。
- 可分别开启或关闭 Codex / Claude Desktop 提醒。
- 可开启或关闭开机自启动。
- 可分别手动测试 Codex / Claude Desktop 提示音。
- 可手动恢复音量。
- 本地日志，方便排查问题。
...
2. 解压整个 zip。
3. 打开 Codex 设置，把 `轮次完成通知` 设为 `始终`；Claude Desktop 使用系统通知即可。
4. 打开 `win-x64` 文件夹。
5. 双击 `CodexNotificationBooster.exe`。
6. 程序会出现在 Windows 右下角托盘区域。
7. 右键托盘图标打开菜单。

首次运行时，Windows 可能会请求通知读取权限。这个权限用于识别 Codex 和 Claude Desktop 通知，不会清除、回复、修改或上传你的通知内容。
...
- 当前没有音效选择器；Codex 和 Claude Desktop 使用各自的内置提示音。
- WSL 只用于开发和构建，不是运行环境。

## 工作原理

程序不是监控 Codex 或 Claude Desktop 进程，而是监听 Windows 通知中心里的 toast 通知。

流程是：

1. Windows 出现一条通知。
2. 程序读取 Windows 暴露的通知元数据。
3. 如果判断为 Codex 或 Claude Desktop 通知，并且对应应用提醒开关打开，就由本工具进程播放对应提示音。
4. 如果开启“音频闪避”，程序会在提示音播放前短暂降低其他可识别播放会话的音量，随后自动恢复。

这意味着：复制到另一台电脑后，只要那台电脑的 Codex 或 Claude Desktop 会正常发 Windows 通知，并且 Windows 授权本工具读取通知，就可以工作。
...
2. Codex 或 Claude Desktop 自己是否能正常发 Windows 通知。
3. Windows 通知中心里是否能看到对应通知。
...
- `已启用` / `已暂停`：开启或暂停目标应用通知增强提示音。
- `Codex 提醒：开` / `Codex 提醒：关`：开启或关闭 Codex 通知提示音。
- `Claude Desktop 提醒：开` / `Claude Desktop 提醒：关`：开启或关闭 Claude Desktop 通知提示音。
- `音频闪避：开` / `音频闪避：关`：开启或关闭短暂降低其他应用音量。
- `开机自启动：开` / `开机自启动：关`：为当前 Windows 用户开启或关闭登录后自动启动。
- `测试 Codex 提示音`：立即播放一次 Codex 内置提示音。
- `测试 Claude 提示音`：立即播放一次 Claude Desktop 内置提示音。
- `恢复音量`：手动尝试恢复被降低的应用音量。
- `打开日志目录`：打开本地日志目录。
- `退出`：关闭程序。
...
- 程序是否启动；
- 通知监听权限状态；
- 是否识别到 Codex 或 Claude Desktop 通知；
- 是否播放提示音；
- 是否执行音频闪避和恢复；
- 是否出现可恢复错误。
...
1. Windows 11 x64。
2. Codex 或 Claude Desktop Windows 应用能正常发通知。
3. 首次运行时允许本工具读取通知。
4. Windows 通知中心里能出现对应通知。

如果另一台电脑上的目标应用 app identity 和本机不同，程序可能识别不到对应通知。此时需要查看本地日志里的应用身份字段，再调整匹配规则。
...
1. 右键托盘图标，点击 `测试 Codex 提示音` 或 `测试 Claude 提示音`。
2. 确认 Windows 主音量不是静音。
3. 确认本工具没有在音量混合器里被静音。
4. 确认 Codex 或 Claude Desktop 通知确实出现在 Windows 通知中心。
...
这正是本工具要解决的问题之一。Windows 自带 toast 声音可能在某些场景下不明显或不播放；本工具用自己的进程播放提示音，并可短暂降低其他应用音量，让目标应用通知更容易被听见。
...
3. 确认托盘图标出现。
4. 右键托盘图标，点击 `测试 Codex 提示音` 和 `测试 Claude 提示音`。
5. 右键托盘图标，点击 `开机自启动：关`，确认菜单变为 `开机自启动：开`。
6. 再点击 `开机自启动：开`，确认菜单变为 `开机自启动：关`，并确认启动项被删除。
7. 分别切换 `Codex 提醒` 和 `Claude Desktop 提醒`，确认关闭的应用通知不会播放提示音。
8. 触发一条 Codex 或 Claude Desktop 通知。
9. 确认对应提示音播放一次，不循环触发。
10. 检查日志没有持续刷 `listener-poll-failed`。
...
后续如果需要安装包、单文件发布、音效选择器或更复杂的通知匹配，可以单独规划。
